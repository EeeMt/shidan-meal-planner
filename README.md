...
> 输入食材，自动安排一周三餐；想吃什么，自动生成采购清单。手机、电脑都能用，数据只保存在你自己的浏览器里。
...
1. **我有食材 → 安排一周**
   在「库存」页登记买回来的菜，一键生成 7 天（可设 5 天）的午餐和晚餐计划。每餐保持一荤一素，晚餐额外配汤，并附每道菜的做法、用时和难度。
...
- 🥗 **一荤一素**：每餐强制荤素搭配，缺荤补荤、缺素补素，晚餐再加一道汤
...
- 🔒 **数据私有**：所有数据仅存浏览器本地，支持 JSON 导出 / 导入迁移
...
- 库存、菜谱、计划、设置全部保存在**浏览器本地**，不会上传到任何服务器
...
- **换手机/电脑后数据没了？** 用「设置 → 导出数据」备份，新设备导入。
